...
export default class shortcuts_cab extends CardType {
  ...
  init(card, container, home) {
    return new Promise((resolve, reject) => {
      // card.carousel = (home.config.skin.persona[`shortcuts_${home.config.skin.persona.time}`]) ? home.config.skin.persona[`shortcuts_${home.config.skin.persona.time}`] : null;
      let shortcutsList = home.config.skin.persona[`shortcuts_${home.config.skin.persona.time}`];
      if (home.config.language === 'german') {
        card.carousel = shortcutsList.filter(c => c.language === 'german');
      }
      if (home.config.language === 'chinese') {
        card.carousel = shortcutsList.filter(c => c.language === 'chinese');
      }
      if (home.config.language === 'spanish') {
        card.carousel = shortcutsList.filter(c => c.language === 'spanish');
      }
      if (home.config.language === '' || home.config.language === 'english') {
        card.carousel = shortcutsList.filter(c => c.language === null) ? shortcutsList.filter(c => c.language === null) : null;
      }
      if (home.dev) {
