...
export default class top_nav extends CardType {
  ...
  init(card, container, home) {
    return new Promise((resolve, reject) => {
      ...
      if (home.dev) {
        ...
        card.program = this.db(home.config.skin.persona.program);
      } else {
        card.image = this.dd('public/home', card.image);
        card.program = this.dd('public/home', home.config.skin.persona.program);
      }
  ...
  init_nav(status, card, container, home) {
    return new Promise((resolve, reject) => {
      container.querySelector(`[data-id=${this.id}] [data-tab="social"]`).onclick = e => {
        this.homepage.init(home, `social`)
          .catch(e => console.log(e));
